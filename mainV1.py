...
# os.environ["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"] = apikey
# Define OpenAI API KEY
os.environ["OPENAI_API_KEY"] = "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"
...
with st.sidebar.expander("🛠️ Settings", expanded=False):
    ...
    PERSONALITY = st.selectbox(
        label="Personality", options=["general-assistant", "philosophical", "witty"]
    )
    ...
    with st.spinner("Generating response..."):
        try:
            ...
            for i in range(len(st.session_state.past)):
                st.write(
                    user_template.replace("{{MSG}}", st.session_state.past[i]),
                    unsafe_allow_html=True,
                )
                st.write(
                    bot_template.replace("{{MSG}}", st.session_state.generated[i]),
                    unsafe_allow_html=True,
                )
                st.write("")
